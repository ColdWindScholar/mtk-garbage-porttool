...
for i in ["pyinstaller"]:
    print(f"Installing {i}")
    _main(['install', i])
local = os.getcwd()
if platform.system() == 'Linux':
    name = 'mtk-garbage-porttool-linux.zip'
else:
    name = 'mtk-garbage-porttool-win.zip'
...
PyInstaller.__main__.run(['-F', 'main.py', '--exclude-module=numpy', '-i', 'logo.ico', '-w' if platform.system() == 'Windows' else '--strip'])
